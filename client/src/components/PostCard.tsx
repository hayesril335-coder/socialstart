...
export function PostCard({post}:{post:Post}){
 ...
 return <article className="post-card">
  ...
  <Link to={`/post/${post.id}`} className="media-wrap">{post.mediaType==='video'?<video src={post.image} muted playsInline/>:<img src={post.image} alt={post.title}/>}<span className={post.mediaType==='live'?'media-label live-label':'media-label'}>{post.mediaType==='live'?'● LIVE':post.username==='alexmorgan'?'YOUR POST':'FEATURED'}</span>{post.mediaType==='live'&&<span className="live-viewers">{post.views} watching</span>}</Link>
