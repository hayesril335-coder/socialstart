...
type AppState = {
  dark:boolean; setDark:(v:boolean)=>void; unread:number; unreadByConversation:Record<string,number>; markConversationRead:(id:string)=>void; points:number; pointsUsed:number; earnPoint:()=>void; viewPoints:{image:number;video:number}; earnViewPoint:(type:'image'|'video')=>void; giftViewPoints:(username:string,type:'image'|'video',amount:number)=>boolean; creatorPoints:Record<string,number>; donatePoints:(username:string,amount:number)=>boolean; balance:number; addFunds:(amount:number)=>void; spendBalance:(amount:number)=>boolean; postMetrics:Record<string,PostMetric>;
  ...
  userPosts:Post[]; publicPosts:Post[]; addUserPost:(post:{title:string;image:string;mediaType?:'image'|'video';postType?:'post'|'story';expiresAt?:number;category?:string;hashtags?:string[];mediaFilter?:string;overlayText?:string;overlayX?:number;overlayY?:number;trimStart?:number;trimEnd?:number})=>void; deletePost:(id:string)=>void;
...
export function AppProvider({children}:{children:ReactNode}) {
  ...
  const readViewPoints=()=>{const id=accountId(),all=read<Record<string,{image:number;video:number}>>('socialstart-view-points-by-account',{}),saved=all[id]||{image:0,video:0};return {image:Math.max(0,saved.image||0,Number(localStorage.getItem(`socialstart-image-view-points-${id}`)||0)),video:Math.max(0,saved.video||0,Number(localStorage.getItem(`socialstart-video-view-points-${id}`)||0))}}
  ...
  useEffect(()=>{const id=accountId(),all=read<Record<string,{image:number;video:number}>>('socialstart-view-points-by-account',{});all[id]=viewPoints;localStorage.setItem('socialstart-view-points-by-account',JSON.stringify(all));localStorage.setItem(`socialstart-image-view-points-${id}`,String(viewPoints.image));localStorage.setItem(`socialstart-video-view-points-${id}`,String(viewPoints.video));scheduleCloudSave()},[viewPoints])
  ...
  const giftViewPoints=(username:string,type:'image'|'video',amount:number)=>{
   ...
   setViewPoints(current=>({...current,[type]:current[type]-amount}))
