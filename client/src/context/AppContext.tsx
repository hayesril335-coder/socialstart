...
type AppState = {
  ...
  userPosts:Post[]; publicPosts:Post[]; addUserPost:(post:{title:string;image:string;mediaType?:'image'|'video';category?:string;hashtags?:string[]})=>void; deletePost:(id:string)=>void;
...
export function AppProvider({children}:{children:ReactNode}) {
  ...
  const addUserPost=({title,image,mediaType='image',category,hashtags=[]}:{title:string;image:string;mediaType?:'image'|'video';category?:string;hashtags?:string[]})=>setUserPosts(current=>{
   ...
   return [{
    id:`mine-${Date.now()}`,author:profile.name||'Alex Morgan',username:profile.username||'alexmorgan',avatar:profile.avatar||'https://images.unsplash.com/photo-1529139574466-a303027c1d8b?w=300&auto=format&fit=crop',image,title,category:category||undefined,hashtags,location:profile.location||'Los Angeles, CA',likes:0,views:'0',followers:'0',following:false,mediaType,ownerAccountId:localStorage.getItem('socialstart-active-account')||undefined
   } as Post,...current]
  })
