...
import { Bell, Maximize, Radio, Square } from 'lucide-react'
...
export function LivePage(){
 const video=useRef<HTMLVideoElement>(null),stage=useRef<HTMLDivElement>(null),stream=useRef<MediaStream|null>(null)
 const [live,setLive]=useState(false),[starting,setStarting]=useState(false),[error,setError]=useState(''),[title,setTitle]=useState(''),[submittedTitle,setSubmittedTitle]=useState(''),[views,setViews]=useState(0)
 ...
 const endLive=()=>{stream.current?.getTracks().forEach(track=>track.stop());if(video.current)video.current.srcObject=null;setLive(false);setViews(0)}
 return <div className="form-page centered"><p className="eyebrow">GO LIVE</p><h1>Share the moment,<br/>as it happens.</h1><div ref={stage} className={live?'live-preview is-live':'live-preview'}><video ref={video} muted playsInline/>{!live&&<><Radio/><span>Your camera will appear here</span></>}{live&&<><i>LIVE</i><b className="live-view-count">{views} views</b><button className="live-fullscreen" onClick={()=>stage.current?.requestFullscreen?.()} aria-label="View livestream fullscreen"><Maximize/></button></>}</div>
  ...
  <div className="live-actions">{live?<button onClick={endLive} className="secondary-btn end-live"><Square/> End live stream</button>:<button onClick={startLive} disabled={starting||!submittedTitle} className="primary-btn"><Radio/> {starting?'Starting…':'Start live stream'}</button>}</div>
 </div>
}
