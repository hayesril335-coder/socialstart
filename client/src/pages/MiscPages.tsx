...
export function LivePage(){
 ...
 return <div className="form-page centered"><p className="eyebrow">GO LIVE</p><h1>Share the moment,<br/>as it happens.</h1><div className={live?'live-preview is-live':'live-preview'}><video ref={video} muted playsInline/>{!live&&<><Radio/><span>Your camera will appear here</span></>}{live&&<i>LIVE</i>}</div><label className="field">Stream title<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="What are you sharing?"/></label>{error&&<p className="camera-error">{error}</p>}<div className="live-actions">{live?<button onClick={endLive} className="secondary-btn end-live"><Square/> End live stream</button>:<button onClick={startLive} disabled={starting||!title} className="primary-btn"><Radio/> {starting?'Starting…':'Start live stream'}</button>}</div></div>
}
