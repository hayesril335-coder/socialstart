...
import { moderateText, validateMediaFile } from '../../lib/moderation'
...
export function MediaCreatePage(){
 ...
 const [preview,setPreview]=useState(''),[title,setTitle]=useState(''),[cameraReady,setCameraReady]=useState(false),[recording,setRecording]=useState(false),[error,setError]=useState(''),[uploading,setUploading]=useState(false)
 ...
 const chooseFile=(file?:File)=>{if(!file)return;const safetyError=validateMediaFile(file);if(safetyError){setError(safetyError);return}if(kind==='Promo'){if(file.type.startsWith('video/')){const check=document.createElement('video');check.preload='metadata';check.onloadedmetadata=()=>{URL.revokeObjectURL(check.src);if(check.duration>30){setError('One For One videos must be 30 seconds or shorter.');return}loadPromoFile(file)};check.src=URL.createObjectURL(file)}else loadPromoFile(file);return}setError('');setPreview(URL.createObjectURL(file))}
 const upload=async()=>{const safetyError=moderateText(title);if(safetyError){setError(safetyError);return}setUploading(true);setError('');try{const media=await uploadMedia(preview,kind.toLowerCase());const isVideo=kind==='Video'||preview.startsWith('data:video')||preview.startsWith('blob:');if(kind==='Promo'){const profile=(()=>{try{return JSON.parse(localStorage.getItem('socialstart-settings-profile')||'{}')}catch{return {}}})();localStorage.setItem(promoKey(),JSON.stringify({title,media,mediaType:isVideo?'video':'image',views:0,author:profile.name||'SocialStart creator',username:profile.username||'creator',avatar:profile.avatar||''} satisfies Promo));scheduleCloudSave();navigate('/create/one-for-one');return}addUserPost({title,image:media,mediaType:isVideo?'video':'image'});navigate('/profile')}catch(uploadError){setError(uploadError instanceof Error?uploadError.message:'The upload failed.');setUploading(false)}}
