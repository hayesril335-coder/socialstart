...
import { Camera, Image, Pencil, Radio, Repeat2, RotateCw, Square, Trash2, Upload, Video, X } from 'lucide-react'
...
export function MediaCreatePage(){
 ...
 const [preview,setPreview]=useState(''),[title,setTitle]=useState(''),[category,setCategory]=useState(''),[hashtags,setHashtags]=useState<string[]>([]),[cameraReady,setCameraReady]=useState(false),[recording,setRecording]=useState(false),[error,setError]=useState(''),[uploading,setUploading]=useState(false),[editing,setEditing]=useState(false),[rotation,setRotation]=useState(0),[filter,setFilter]=useState('none')
 ...
 return <div className="form-page"><p className="eyebrow">CREATE / {kind.toUpperCase()}</p><h1>{kind==='Photo'?'Take a picture':kind==='Video'?'Record a video':`Share a new ${kind.toLowerCase()}`}</h1>
  ...
  {preview&&<><button className="secondary-btn media-edit-button" onClick={()=>setEditing(true)}><Pencil/> Edit {kind==='Video'?'video':'image'}</button><label className="field">Title<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Give your post a title…"/></label>{kind==='Story'&&<p className="form-note">Your story will disappear after 24 hours.</p>}<div className="form-actions"><button className="secondary-btn" disabled={uploading} onClick={()=>{setPreview('');if(isCamera)navigate(0)}}><X/> Retake</button><button className="primary-btn" disabled={!title||uploading} onClick={upload}>{uploading?'Uploading…':`Upload ${kind}`}</button></div></>}
  {editing&&<div className="purchase-modal media-editor-modal"><div><button className="modal-close" onClick={()=>setEditing(false)}><X/></button><Pencil/><h2>Edit {kind==='Video'?'video':'image'}</h2>{kind==='Video'?<><video src={preview} controls style={{filter}}/><label className="field">Video look<select value={filter} onChange={event=>setFilter(event.target.value)}><option value="none">Original</option><option value="grayscale(1)">Black & white</option><option value="sepia(.75)">Warm</option><option value="saturate(1.5)">Vivid</option><option value="brightness(1.15)">Bright</option></select></label></>:<><img src={preview} style={{filter,transform:`rotate(${rotation}deg)`}}/><button className="secondary-btn" onClick={()=>setRotation(current=>(current+90)%360)}><RotateCw/> Rotate</button><label className="field">Photo look<select value={filter} onChange={event=>setFilter(event.target.value)}><option value="none">Original</option><option value="grayscale(1)">Black & white</option><option value="sepia(.75)">Warm</option><option value="saturate(1.5)">Vivid</option><option value="brightness(1.15)">Bright</option></select></label></>}<button className="primary-btn wide" onClick={()=>setEditing(false)}>Apply edits</button></div></div>}
