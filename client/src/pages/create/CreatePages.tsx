...
export function MediaCreatePage(){
 ...
 const chooseFile=(file?:File)=>{if(!file)return;if(kind==='Promo'){if(file.type.startsWith('video/')){const check=document.createElement('video');check.preload='metadata';check.onloadedmetadata=()=>{URL.revokeObjectURL(check.src);if(check.duration>30){setError('One For One videos must be 30 seconds or shorter.');return}loadPromoFile(file)};check.src=URL.createObjectURL(file)}else loadPromoFile(file);return}setPreview(URL.createObjectURL(file))}
 const upload=()=>{if(kind==='Promo'){const profile=(()=>{try{return JSON.parse(localStorage.getItem('socialstart-settings-profile')||'{}')}catch{return {}}})();localStorage.setItem(promoKey(),JSON.stringify({title,media:preview,mediaType:preview.startsWith('data:video')?'video':'image',views:0,author:profile.name||'SocialStart creator',username:profile.username||'creator',avatar:profile.avatar||''} satisfies Promo));navigate('/create/one-for-one');return}addUserPost({title,image:preview,mediaType:kind==='Video'?'video':'image'});navigate('/profile')}
