...
type Promo={title:string;media:string;mediaType:'image'|'video';views:number;author:string;username:string;avatar:string;category?:string}
const promoKey=()=>`socialstart-account-promo-${localStorage.getItem('socialstart-active-account')||'guest'}`
...
export function MediaCreatePage(){
 ...
 const [preview,setPreview]=useState(''),[title,setTitle]=useState(''),[category,setCategory]=useState(''),[cameraReady,setCameraReady]=useState(false),[recording,setRecording]=useState(false),[error,setError]=useState(''),[uploading,setUploading]=useState(false)
 ...
 const chooseFile=async(file?:File)=>{if(!file)return;const safetyError=validateMediaFile(file);if(safetyError){setError(safetyError);return}const source=URL.createObjectURL(file);if(file.type.startsWith('video/')){try{const duration=await readVideoDuration(source),maximum=kind==='Promo'?30:600;if(duration>maximum){setError(kind==='Promo'?'One For One videos must be 30 seconds or shorter.':'Uploaded videos must be 10 minutes or shorter.');URL.revokeObjectURL(source);return}}catch(durationError){setError(durationError instanceof Error?durationError.message:'The video duration could not be verified.');URL.revokeObjectURL(source);return}}if(kind==='Promo'){loadPromoFile(file);URL.revokeObjectURL(source);return}setError('');setPreview(source)}
 const upload=async()=>{const safetyError=moderateText(title);if(safetyError){setError(safetyError);return}setUploading(true);setError('');try{const isVideo=kind==='Video'||preview.startsWith('data:video')||preview.startsWith('blob:');if(isVideo){const duration=await readVideoDuration(preview),maximum=kind==='Promo'?30:600;if(duration>maximum)throw new Error(kind==='Promo'?'One For One videos must be 30 seconds or shorter.':'Uploaded videos must be 10 minutes or shorter.')}const media=await uploadMedia(preview,kind.toLowerCase());if(kind==='Promo'){const profile=(()=>{try{return JSON.parse(localStorage.getItem('socialstart-settings-profile')||'{}')}catch{return {}}})();localStorage.setItem(promoKey(),JSON.stringify({title,media,category:category||undefined,mediaType:isVideo?'video':'image',views:0,author:profile.name||'SocialStart creator',username:profile.username||'creator',avatar:profile.avatar||''} satisfies Promo));scheduleCloudSave();navigate('/create/one-for-one');return}addUserPost({title,image:media,category:category||undefined,mediaType:isVideo?'video':'image'});navigate('/profile')}catch(uploadError){setError(uploadError instanceof Error?uploadError.message:'The upload failed.');setUploading(false)}}
