...
type PointBalances=Record<string,{image:number;video:number}>
const pointBalances=()=>{try{return JSON.parse(localStorage.getItem('socialstart-view-points-by-account')||'{}') as PointBalances}catch{return {}}}
const readPoints=(type:'image'|'video')=>pointBalances()[account()]?.[type]??Number(localStorage.getItem(pointKey(type))||0)
const savePoints=(type:'image'|'video',value:number)=>{const balances=pointBalances(),current=balances[account()]||{image:0,video:0};balances[account()]={...current,[type]:value};localStorage.setItem('socialstart-view-points-by-account',JSON.stringify(balances));localStorage.setItem(pointKey(type),String(value));scheduleCloudSave()}
...
export function OneForOneDashboard(){
 const [photoPoints]=useState(()=>readPoints('image')),[videoPoints]=useState(()=>readPoints('video'))
...
export function EarnViewPointsPage(){
 ...
 const completeVideo=()=>{setPoints(current=>{const next=current+1;savePoints(type,next);return next});setResult('+1 video view point');window.setTimeout(()=>setIndex(current=>current+1),700)}
 ...
 const verify=()=>{if(Number(answer)!==digit){setResult('Incorrect');return}setPoints(current=>{const next=current+1;savePoints(type,next);return next});setResult('+1 point');window.setTimeout(()=>setIndex(current=>current+1),650)}
 return <div className="form-page earn-view-page"><p className="eyebrow">{type==='image'?'PHOTO':'VIDEO'} VIEW POINTS · {formatCount(points)}</p><div className="earn-view-card"><header><b>{promo.title}</b><strong>{type==='image'?`${seconds}s`:'Next video plays automatically'}</strong></header><div className="earn-media">{type==='video'?<video key={`${index}-${promo.media}`} src={promo.media} autoPlay controls playsInline onEnded={completeVideo}/>:<img src={promo.media} alt={promo.title}/>} {showDigit&&<i className="verification-digit">{digit}</i>}</div><footer><button className={liked?'active':''} onClick={()=>setLiked(!liked)}><Heart fill={liked?'currentColor':'none'}/></button><button className={saved?'active':''} onClick={()=>setSaved(!saved)}><Bookmark fill={saved?'currentColor':'none'}/></button><button className="follow" onClick={()=>toggleFollow(promo.username)}><UserPlus/> {followingUsernames.includes(promo.username)?'Following':'Follow'}</button></footer></div>{type==='video'&&result&&<div className="share-success">{result} · Loading next video…</div>}{ready&&<div className="verification-box"><label>What number appeared?<input autoFocus inputMode="numeric" maxLength={1} value={answer} onChange={event=>setAnswer(event.target.value.replace(/\D/g,'').slice(0,1))}/></label><button className="primary-btn" onClick={verify} disabled={!answer}>Submit</button>{result&&<b className={result==='Incorrect'?'incorrect':''}>{result}</b>}</div>}</div>
}
