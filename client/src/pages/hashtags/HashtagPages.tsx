...
import { extractHashtags, followedHashtags, followHashtag, hashtagRanks, hashtagRegistry, ownedHashtags, rankBoostHours, rankFor, rankPriority, setHashtagRank, unfollowHashtag, type HashtagRank } from '../../lib/hashtags'
...
export function CreatedHashtagsPage(){
 const username=ownUsername(),[created,setCreated]=useState(()=>ownedHashtags(username))
 ...
 return <div className="form-page hashtag-list-page"><p className="eyebrow">HASHTAGS YOU FOUNDED · {created.length}/10</p><h1>Created Hashtags</h1>{created.length?<div className="hashtag-list">{created.map(record=><div key={record.tag}><Link to={`/hashtag/${record.tag}`}><Hash/><span><b>#{record.tag}</b><small>Founder · Manage ranks</small></span></Link></div>)}</div>:<div className="profile-empty"><Users/><h3>No created hashtags yet</h3><p>Be the first person to use a hashtag in a post to become its founder.</p></div>}</div>
}
...
export function HashtagPage(){
 ...
 const all=[...userPosts,...publicPosts,...posts].filter((post,index,array)=>array.findIndex(item=>item.id===post.id)===index&&extractHashtags(post.title).includes(tag.toLowerCase()))
