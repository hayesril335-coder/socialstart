...
export function HashtagPage(){
 ...
 const normalizedTag=decodeURIComponent(tag).replace(/^#/,'').trim().toLowerCase()
 const all=[...userPosts,...publicPosts,...posts].filter((post,index,array)=>array.findIndex(item=>item.id===post.id)===index&&[...(post.hashtags||[]),...extractHashtags(post.title)].some(item=>item.replace(/^#/,'').trim().toLowerCase()===normalizedTag))
