...
export function ProfilePage(){
 ...
 return <div className="profile-page">
  ...
  {own?(visiblePosts.length===0?<div className="profile-empty"><Grid3X3/><h3>{tab==='Posts'?'No posts yet':'Nothing saved yet'}</h3><p>{tab==='Posts'?'When you share your first post, it will appear here.':'Posts you save will appear here.'}</p>{tab==='Posts'&&<Link to="/create" className="primary-btn">Create your first post</Link>}</div>:<div className="profile-grid">{visiblePosts.map(p=><Link to={`/post/${p.id}`} key={p.id}>{p.mediaType==='video'?<video src={p.image} muted playsInline/>:<img src={p.image}/>}<span>♥ {p.likes+(likedPostIds.includes(p.id)?1:0)} · ◉ {p.views}</span></Link>)}</div>):<div className="profile-grid">{posts.filter(p=>p.username===profile.user).map(p=><Link to={`/post/${p.id}`} key={p.id}><img src={p.image}/><span>♥ {p.likes.toLocaleString()} · ◉ {p.views}</span></Link>)}</div>}
 </div>
}
