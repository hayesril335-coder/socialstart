...
export function ProfilePage(){
 ...
 const {dark,setDark,balance,points,pointsUsed,creatorPoints,userPosts,savedPosts,likedPostIds,followingUsernames,toggleFollow,shareCount}=useApp()
 ...
 return <div className="profile-page">
  ...
  <section className="stat-strip"><div><b>{stats[0]}</b><span>Followers</span></div><div><b>{stats[1]}</b><span>Following</span></div><div><b>{stats[2]}</b><span>Total likes</span></div><div><b>{stats[3]}</b><span>Views</span></div>{own&&<><div><b>{shareCount}</b><span>Shares</span></div><div><b>{points}</b><span>Social points</span></div><div><b>{pointsUsed}</b><span>Points used</span></div><div><b>${balance.toFixed(2)}</b><span>Balance</span></div></>}</section>
