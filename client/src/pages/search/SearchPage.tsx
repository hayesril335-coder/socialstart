...
export function SearchPage(){
 ...
 return <div className="feed-page">
  <section className="feed-intro"><p className="eyebrow">DISCOVER YOUR WORLD</p><h1>What’s happening<br/><em>around you?</em></h1><div className="feed-tabs">{['Following','Popular','Nearby'].map(item=><button onClick={()=>selectFeed(item)} className={feed===item?'active':''} key={item}>{item==='Nearby'&&<MapPin/>}{item}</button>)}</div>{feed==='Nearby'&&locationStatus&&<p className="location-status">{locationStatus}</p>}<label className="search-box"><Search/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search creators, stories, or ideas"/><SlidersHorizontal/></label></section>
